...
class poetryModel:
    ...
    def accuracy_model(self,probs,targets,words_size):
        targets_one_hot = tf.one_hot(targets, words_size)
        targets_reshaped = tf.reshape(targets_one_hot,[-1,words_size])
        correct_prediction = tf.equal(tf.argmax(probs,1),tf.argmax(targets_reshaped,1))
