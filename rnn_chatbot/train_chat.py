...
if __name__ == '__main__':
    ...
    logger = logging.getLogger("train_chat")
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False,gpu_options=gpu_options)) as sess:
        ...
        while True:
            ...
            logger.debug("step:%d,loss:%f" % (step,step_loss))
            loss += step_loss / 2000
            step += 1
            if step % 1000 == 0:
                logger.debug("step:%d,per_loss:%f" % (step,loss))
            ...
            '''if step % 5000 == 0:
                for bucket_id in range(len(generate_chat._buckets)):
                    if len(test_set[bucket_id]) == 0:
                        ...
                        logger.debug("bucket_id:%d,eval_loss:%f" % (bucket_id,eval_loss))'''
